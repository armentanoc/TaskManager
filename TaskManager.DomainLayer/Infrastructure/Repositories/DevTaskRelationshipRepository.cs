...
namespace TaskManager.DomainLayer.Infrastructure.Repositories
{
    internal static class DevTaskRelationshipRepository
    {
        ...
        {
            InsertTaskIfNotExists(connection, new DevTaskRelationship(
                parentOrFirst: "1",
                childOrSecond: "2",
        ...
        internal static void InitializeNewDevTaskRelationship(DevTaskRelationship newRelation)
        {
            ...
            try
            {
                ...
                InsertTaskIfNotExists(defaultConnection, newRelation);
            }
            catch (Exception ex)
            {
        ...
        public static void AddDevTaskToDatabase(DevTaskRelationship relationship)
        {
            DatabaseConnection.ExecuteWithinTransaction((transactionConnection) =>
            {
                try
                {
                    InsertTaskIfNotExists(transactionConnection, relationship);
                }
                catch (Exception ex)
                {
        ...
        internal static void InsertTaskIfNotExists(SQLiteConnection connection, DevTaskRelationship relationship)
        {
            try
            {
        ...
        private static bool DoesRelationshipExist(SQLiteConnection connection, DevTaskRelationship relationship)
        {
            ...
            string query = $"SELECT COUNT(*) FROM {TableName} " +
                $"WHERE " +
                $"(ParentOrFirst_DevTask_Id == @ParentOrFirst" +
                $"AND ChildOrSecond_DevTask_Id == @ChildOrSecond)" +
                $"OR (ParentOrFirst_DevTask_Id == @ChildOrSecond" +
                $"AND ChildOrSecond_DevTask_Id == @ParentOrFirst);";
